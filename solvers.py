...
class GradientDescentSolver(Solver):
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        for i in range(len(param_vars)):
            velocity_update = self.momentum * vel_vars[i] - self.learning_rate * grad_tensors[i]
            tensor_update = param_vars[i] + velocity_update
            updates.append((vel_vars[i],velocity_update))
            updates.append((param_vars[i],tensor_update))
        return updates
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        # You may want to initialize some variables that are shared across iterations
        "*** YOUR CODE HERE ***"
        training_generator = MinibatchIndefinitelyGenerator(train_data, 1 , self.shuffle)
        ...
        for iter_ in range(self.iterations):
            ...
            p,q = training_generator.next()
            train_loss = session.run(loss_tensor,feed_dict= {model.input_ph: p, target_ph: q})
            session.run(update_ops, feed_dict= {model.input_ph: p, target_ph: q})
            x, y = value_generator.next()
            val_loss = session.run(loss_tensor, feed_dict={model.input_ph: x, target_ph: y})
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        # You may want to initialize some variables that are shared across iterations
        "*** YOUR CODE HERE ***"
        training_generator = MinibatchIndefinitelyGenerator(train_data, self.batch_size, self.shuffle)
