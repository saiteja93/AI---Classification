...
def search_hyperparams(train_data, train_labels, val_data, val_labels,
    ...
    optimal_hyperparams, i, optimal_model, maximum_accuracy = None, 1, None, float("-inf")

    for model, (learning_rate, momentum, batch_size) in zip(models, hyperparams):
        "*** YOUR CODE HERE ***"
        sgd = solvers.MinibatchStochasticGradientDescentSolver(learning_rate,iterations,batch_size,momentum)
        train_error, value_error = sgd.solve(train_data,train_labels, val_data, val_labels, model)
        model_accuracy = model.accuracy(val_data, val_labels)
        if model_accuracy> maximum_accuracy:
            maximum_accuracy = model_accuracy
            optimal_model = model
            optimal_hyperparams = i
        i += 1

    return optimal_model, optimal_hyperparams
